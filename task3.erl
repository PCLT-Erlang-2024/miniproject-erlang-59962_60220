-module(task3).
-export([start/0, initConveyors/1, initTrucks/3, loopProducer/1]).
...
initTrucks(Belts, Size, Time) when is_integer(Belts) and is_integer(Size) and is_integer(Time) ->
    Trucks = {get_List({Belts, Size}), []},
    io:format("Created trucks in ~p~n", [whereis(trucks)]),
    loopTrucks(Trucks, Size, Time).
        ...
        {request, _, {add_package, Pid, ClusterID, Size}} ->
            ...
            case allocate_truck(Trucks, ClusterID) of
                {NewTrucks, {ok, {ClusterID, S}}} when S >= Size ->
                    ...
                    loopTrucks(UpdatedTrucks, Max, Time);
